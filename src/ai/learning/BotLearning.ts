...
export class BotLearning {
	...
	private readonly lastUnstable = new Map<number, boolean>();
	private readonly lastJumped = new Map<number, boolean>();
	private matchAerialTouches = 0;
	private matchUnstableSamples = 0;
	private matchObserveSamples = 0;
	private readonly matchRecoveryDurations: number[] = [];
	private readonly unstableSinceMs = new Map<number, number>();
	private readonly smoothPolicyYaw = new Map<number, number>();
	private lastMatchRecoveryEfficiency = 1;
	private readonly scratchObs = new Float32Array(21);
	private readonly scratchOut = new Float32Array(12);
	...
	getLastMatchRecoveryEfficiency(): number {
		return this.lastMatchRecoveryEfficiency;
	}
	...
	/** Na początku ticku — predykcja sieci + tuning celu jazdy (ten sam frame). */
	beginThink(
		slotIndex: number,
		player: Player,
		role: BotRole,
		...
		if (!this.enabled) return NEUTRAL_TUNING;

		const pol = this.activePolicy();
		buildBotObservation(player, { ...ctx, role }, this.scratchObs);
		...
		this.tuningBySlot.set(slotIndex, tuning);
	...
	think(
		...
		const step = this.stepMemory.get(slotIndex);
		if (!step) return heuristic;

		this.smoothNetworkSteer(slotIndex, step.outputs);
		...
		if (!active) {
			this.lastJumped.set(slotIndex, heuristic.jump === true);
			return heuristic;
		}

		if (ctx.forceRecovery || isBotUnstable(player)) {
			const recoveryDrive: BotDrive = {
				...heuristic,
				boost: false,
				shift: heuristic.shift === true,
			};
			this.lastJumped.set(slotIndex, recoveryDrive.jump === true);
			return recoveryDrive;
		}
	...
	onMatchEnd(blueScore: number, orangeScore: number, cars: CarEntity[]): void {
		...
		this.lastMatchRecoveryEfficiency = computeMatchRecoveryEfficiency(
			this.matchUnstableSamples,
			this.matchObserveSamples,
			this.matchRecoveryDurations,
		);
		this.matchAerialTouches = 0;
		this.matchUnstableSamples = 0;
		this.matchObserveSamples = 0;
		this.matchRecoveryDurations.length = 0;
		this.unstableSinceMs.clear();
		this.recordMatchProgress(
			botDelta,
			aerialTouches,
			this.lastMatchRecoveryEfficiency,
		);
		this.clearStepTracking();
	...
	private computeStepReward(
		...
		const unstable = isBotUnstable(player);
		const wasUnstable = this.lastUnstable.get(slotIndex) ?? false;
		this.lastUnstable.set(slotIndex, unstable);
		this.matchObserveSamples++;
		if (unstable) {
			this.matchUnstableSamples++;
			if (!this.unstableSinceMs.has(slotIndex)) {
				this.unstableSinceMs.set(slotIndex, Date.now());
			}
		} else if (wasUnstable && isBotStableOnWheels(player)) {
			const start = this.unstableSinceMs.get(slotIndex);
			if (start !== undefined) {
				this.matchRecoveryDurations.push((Date.now() - start) / 1000);
				this.unstableSinceMs.delete(slotIndex);
			}
		} else if (!unstable) {
			this.unstableSinceMs.delete(slotIndex);
		}
		...
		if (inAir && ctx.ballPos.y > 1.25 && dist < 3.5) {
			reward += 0.04;
			const fx = player.getForward().x;
			const fz = player.getForward().z;
			const tx = ctx.ballPos.x - pos.x;
			const tz = ctx.ballPos.z - pos.z;
			const fLen = Math.hypot(fx, fz);
			const tLen = Math.hypot(tx, tz);
			if (fLen > 1e-3 && tLen > 1e-3) {
				const align = (fx / fLen) * (tx / tLen) + (fz / fLen) * (tz / tLen);
				if (align > 0.55) reward += 0.06 * align;
			}
		}
	...
	private clearStepTracking(): void {
		...
		this.lastUnstable.clear();
		this.lastJumped.clear();
		this.smoothPolicyYaw.clear();
	}

	/** Low-pass na wyjściu sieci — redukuje jitter yaw (kręcenie w kółko). */
	private smoothNetworkSteer(slotIndex: number, outputs: Float32Array): void {
		const rawYaw = outputs[5] ?? 0;
		const prev = this.smoothPolicyYaw.get(slotIndex) ?? rawYaw;
		const smoothed = THREE.MathUtils.lerp(prev, rawYaw, 0.24);
		this.smoothPolicyYaw.set(slotIndex, smoothed);
		outputs[5] = smoothed;
	}
