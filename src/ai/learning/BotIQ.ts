...
export type BotIQInputs = {
	...
	/** Średnia skuteczność recovery 0..1 (ostatnie mecze). */
 ...
 * Składniki: fitness, best, generacja, forma (botDelta), aerial, recovery, micro-evolve.
 */
export function computeBotIQ(inputs: BotIQInputs): number {
	...
	const recoveryNorm = clamp(inputs.recoveryEfficiency, 0, 1);
...
export function iqFromProgressEntry(entry: BotProgressEntry): number {
	...
	return computeBotIQ({
		...
		recoveryEfficiency: entry.recoveryEfficiency ?? 1,
