...
export const POLICY_INPUT_SIZE = 18;
export const POLICY_HIDDEN_SIZE = 20;
export const POLICY_OUTPUT_SIZE = 4;
...
export class BotPolicy {
	...
	driveFromOutputs(outputs: Float32Array): BotDrive {
		const forwardRaw = outputs[0]!;
		const yawRaw = outputs[1]!;
		const boostRaw = outputs[2]!;
		const jumpRaw = outputs[3]!;

		let forward = 0;
		if (forwardRaw > 0.22) forward = 1;
		else if (forwardRaw < -0.22) forward = -1;

		let yaw = 0;
		if (yawRaw > 0.18) yaw = 1;
		else if (yawRaw < -0.18) yaw = -1;

		return {
			forward,
			yaw,
			boost: sigmoid(boostRaw) > 0.52,
			jump: sigmoid(jumpRaw) > 0.62,
		};
	}
