...
export type BotProgressEntry = {
	...
	/** 0..1 — skuteczność powrotu na 4 koła po niestabilnym lądowaniu. */
...
export function buildProgressIQSnapshot(
	...
	const recoveryEntries = matchEntries.filter(
		(e) => e.recoveryEfficiency !== undefined,
	);
	const recoveryEfficiency =
		recoveryEntries.length > 0
			? recoveryEntries.reduce((s, e) => s + (e.recoveryEfficiency ?? 0), 0) /
