...
/** Kierunek dodge w przestrzeni sticka RL: (-pitch, yaw). */
export function rlDodgeDirection(stick: DodgeStick): {
	dodgeX: number;
	dodgeY: number;
} {
	const inputMag = Math.abs(stick.throttle) + Math.abs(stick.yaw);
	if (inputMag < 0.01) return { dodgeX: 0, dodgeY: 0 };

	const len = Math.hypot(stick.throttle, stick.yaw);
	if (len < 0.01) return { dodgeX: 0, dodgeY: 0 };

	let dodgeX = stick.throttle / len;
	let dodgeY = stick.yaw / len;
	if (Math.abs(dodgeX) < 0.1) dodgeX = 0;
	if (Math.abs(dodgeY) < 0.1) dodgeY = 0;
	// Dominująca oś — ignoruj szum drugiej (keyboard ghost A/D przy W).
	if (Math.abs(dodgeX) > 0.85 && Math.abs(dodgeY) < 0.2) dodgeY = 0;
	if (Math.abs(dodgeY) > 0.85 && Math.abs(dodgeX) < 0.2) dodgeX = 0;
	const snapLen = Math.hypot(dodgeX, dodgeY);
	if (snapLen > 0.01) {
		dodgeX /= snapLen;
		dodgeY /= snapLen;
	}
...
/** Relatywne składowe torque flipa (RocketSim flipRelTorque). */
export function rlFlipRelTorque(stick: DodgeStick): {
	pitch: number;
	roll: number;
} {
	const { dodgeX, dodgeY } = rlDodgeDirection(stick);
	return { pitch: dodgeX, roll: -dodgeY };
}

/**
 * Składowe impulsu dodge (m/s) wzdłuż poziomych osi auta.
 * Port RocketSim / Rocket Science #14 — 500 uu/s bazowo, skale prędkości.
 */
export function rlDodgeImpulseComponentsMps(
	...
	const { dodgeX, dodgeY } = rlDodgeDirection(stick);
	if (dodgeX === 0 && dodgeY === 0) return { alongFwd: 0, alongSide: 0 };

	const forwardSpeedRatio = Math.min(
		Math.abs(forwardSpeedAlongNoseUu) / RL_MAX_CAR_SPEED_UU,
		1,
	);

	let shouldDodgeBackwards: boolean;
	if (Math.abs(forwardSpeedAlongNoseUu) < 100) {
		shouldDodgeBackwards = dodgeX < 0;
	} else {
		shouldDodgeBackwards = dodgeX >= 0 !== forwardSpeedAlongNoseUu >= 0;
	}

	let velX = dodgeX * 5.0;
	let velY = dodgeY * 5.0;

	const maxSpeedScaleX = shouldDodgeBackwards ? 2.5 : 1.0;
	velX *= (maxSpeedScaleX - 1) * forwardSpeedRatio + 1;
	velY *= (1.9 - 1) * forwardSpeedRatio + 1;

	if (shouldDodgeBackwards) {
		velX *= 16 / 15;
	}

	return { alongFwd: velX, alongSide: velY };
}

/** Skala anulowania pitch torque podczas flip cancel (0 = pełny cancel). */
export function rlFlipCancelPitchScale(
	flipRelPitch: number,
	pitchInput: number,
	flipElapsed: number,
	noCancelTime: number,
): number {
	if (flipRelPitch === 0 || pitchInput === 0) return 1;
	if (flipElapsed < noCancelTime) return 1;
	const sameSign =
		Math.sign(flipRelPitch) === Math.sign(pitchInput) ||
		(flipRelPitch === 0 && pitchInput === 0);
	if (!sameSign) return 1;
	return 1 - Math.min(Math.abs(pitchInput), 1);
}
