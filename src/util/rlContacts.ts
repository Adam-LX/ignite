...
import { RL_BALL, RL_CAR } from "./rlConstants";
...
export function stabilizePlayerPhysics(player: Player): void {
	...
	const boundsX = RL_ARENA.HALF_WIDTH + 8;
	const boundsZ = RL_ARENA.HALF_LENGTH + RL_ARENA.GOAL_DEPTH + 8;
	const boundsY = RL_ARENA.HEIGHT + 6;

	if (
		isFiniteVec3(t) &&
		Math.abs(t.x) <= boundsX &&
		Math.abs(t.z) <= boundsZ &&
		t.y >= -2 &&
		t.y <= boundsY
	) {
...
export function applyCarBallHits(
	...
	world.contactPair(player.rapierCollider, ball.rapierCollider, () => {
		...
		if (
			closing < RL_BALL.minApproachSpeed * 0.5 &&
			relSpeedUu < mpsToUu(RL_BALL.minHitSpeed)
		) {
			return;
		}
		...
		clampLinvel(ballBody, MAX_BALL_SPEED);
		clampBallVertSpeed(
			ballBody,
			player.isBoosting()
...
export function updateBallPhysics(
	...
	clampLinvel(body, MAX_BALL_SPEED);
}
