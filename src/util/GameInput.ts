...
import { RL_CAR } from "./rlConstants";
...
class GameInput implements ControlInput {
	...
	private quickChatHeld = false;
	private prevJumpBtn = false;
	private prevBallCamBtn = false;
	private minimapToggleQueued = false;
	...
	private constructor() {
		const onKeyDown = (e: KeyboardEvent) => {
			...
			if (e.code === "KeyT") {
				e.preventDefault();
				this.quickChatHeld = true;
				return;
			}
			if (e.code === "KeyM") {
				if (!e.repeat) this.minimapToggleQueued = true;
				return;
			}
			this.applyKeyCode(e.code, true);
			this.applyLegacyKey(e.key, true);
			/** Sam e.shiftKey — NIE `|| this.shift` (zostawał wciśnięty → A/D = roll zamiast yaw). */
	...
	consumeMinimapToggle(): boolean {
		if (!this.minimapToggleQueued) return false;
		this.minimapToggleQueued = false;
		return true;
	}
	...
	isQuickChatHeld(): boolean {
		return this.quickChatHeld;
	}
	...
	yaw(): number {
		...
		const kb =
			(Number(this.inputs.left) - Number(this.inputs.right)) *
			RL_CAR.keyboardYawScale;
