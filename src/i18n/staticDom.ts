...
export function renderCreditsCard(): void {
    ...
    <h3>${t("credits.changelog")}</h3>
    <p class="credits-changelog">${t("credits.changelog.body")}</p>

    <h3>${t("credits.typography")}</h3>
    <ul>
      <li>${t("credits.typography.orbitron")}</li>
      <li>${t("credits.typography.wallpoet")}</li>
    </ul>
    ...
    <button type="button" id="credits-close" class="ui-ignite-back credits-panel__back" aria-label="${t("ui.back")}">
      <span class="ui-ignite-back__shine" aria-hidden="true"></span>
      <span class="ui-ignite-back__icon" aria-hidden="true">←</span>
      <span class="ui-ignite-back__label">${t("ui.back")}</span>
    </button>
  `;
