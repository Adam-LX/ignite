...
import { getMatchDurationSec, getModePolicy } from "../game/modePolicy";
import { getModeSpec, MATCH_RULES } from "../game/modes";
import { t } from "../i18n";
import { MatchScoring } from "../match/MatchScoring";
import { forfeitMatchScores } from "../net/rankedForfeit";
...
import {
	duelRivalDisplayName,
	ignitionFfaDisplayName,
} from "../game/botDisplayNames";
import { buildRlKickoffSpawns } from "../game/rlKickoffSpawns";
import { defaultSpawnCenterY } from "../visual/carWheelGround";
import { meridianSphereRadius } from "../visual/meridianArena";

const IGNITION_RADIUS = 14;

const IGNITE_FLASH_SEC = 0.4;
...
export function buildSpawnPositions(
	...
	if (mode === "meridian2v2") {
		return buildMeridianSpawns(carHalfHeight);
	}
...
export class MatchController {
	...
	private readonly skipGoals: boolean;
	private readonly touchTracker = new BallTouchTracker();
	readonly scoring = new MatchScoring();
	...
	private forfeitLoserSlot: number | null = null;

	constructor(scene: THREE.Scene, mode: GameModeId) {
		this.goalVfx = new GoalVfx(scene);
		this.mode = mode;
		this.isFFA = getModeSpec(mode).isFFA;
		this.skipGoals = getModePolicy(mode).features.meridian;
		this.timeRemainingSec = getMatchDurationSec(mode);
	}
	...
	update(
		...
		this.scoring.applyGoal(this.pendingGoalMoment.touch, cars, this.isFFA);

		getStadiumLeds()?.onGoal(scored);
		const goalPos = this.goalCenter(scored);
		this.goalVfx.trigger(goalPos, scored);
		triggerGoalNetRipple(scored === "blue" ? "orange" : "blue");
	...
	/** Gość online — synchronizacja fazy z snapshotem hosta. */
	syncGuestReplayPhase(active: boolean): void {
		if (active) {
			if (this.phase !== "goal_replay") {
				this.phase = "goal_replay";
			}
	...
	/** Online — poddanie meczu (3–0 dla zwycięzcy). */
	applyOnlineForfeit(loserSlot: number): void {
		if (this.phase === "finished") return;
		const scores = forfeitMatchScores(loserSlot, this.mode);
		this.blueScore = scores.blueScore;
		this.orangeScore = scores.orangeScore;
		this.forfeitLoserSlot = loserSlot;
		this.phase = "finished";
		this.goalLock = null;
		this.finishAfterGoal = false;
		this.skipGoalReplay = true;
	}

	wasForfeit(): boolean {
		return this.forfeitLoserSlot !== null;
	}
	...
	shouldRecordReplay(): boolean {
		/** Pre-gol w playing + post-gol w bounce (REPLAY_POST_SEC). */
		return this.phase === "playing" || this.phase === "goal_bounce";
	}
	...
	triggerReplayGoalVfx(team: ScoringTeam): void {
		this.goalVfx.trigger(this.goalCenter(team), team);
	}
	...
	/** DEV/audit — natychmiast kończy odliczanie kickoffu. */
	forceStartPlaying(): void {
		this.countdownHold = false;
		this.countdownLeft = 0;
		this.kickoffIgniteLeft = 0;
	...
	/** DEV/audit — skróć bounce i wejdź w replay. */
	forceEnterGoalReplay(): void {
		if (this.phase === "goal_bounce" || this.phase === "playing") {
			this.goalBounceLeft = 0;
			this.skipGoalReplay = false;
			this.phase = "goal_replay";
		}
	}
	...
	getHudSnapshot(cars: CarEntity[]): MatchHudSnapshot {
		...
		return {
			...
			countdownSec:
				this.countdownHold || this.phase !== "countdown"
					? null
					: Math.max(0, this.countdownLeft),
			kickoffTick: this.getKickoffTick(),
			kickoffIgnite: !this.countdownHold && this.kickoffIgniteLeft > 0,
	...
	private getKickoffTick(): number | null {
		if (this.countdownHold) return null;
	...
	/** Meridian — punkty possession bez goal pause. */
	addPossessionPoints(blueDelta: number, orangeDelta: number): void {
		if (blueDelta > 0) this.blueScore += blueDelta;
		if (orangeDelta > 0) this.orangeScore += orangeDelta;
	}

	isMeridianMode(): boolean {
		return this.skipGoals;
	}
	...
	getLastTouchTeam(cars: CarEntity[]): ScoringTeam | null {
		const slot = this.touchTracker.lastTouchSlotId;
		if (slot === null) return null;
		return cars.find((c) => c.slotIndex === slot)?.team ?? null;
	}
	...
	private resolveWinner(cars: CarEntity[]): string {
		if (this.forfeitLoserSlot !== null) {
			return this.blueScore > this.orangeScore
				? t("match.blueForfeitWin")
				: t("match.orangeForfeitWin");
		}
		if (this.isFFA) {
			const top = [...cars].sort(
...
function buildIgnition1v1Spawns(carHalfHeight: number): CarSpawn[] {
	return buildRlKickoffSpawns("1v1", carHalfHeight).map((spawn) => ({
		...spawn,
		team: null,
		displayName: spawn.slotIndex === 0 ? "You" : duelRivalDisplayName(),
	}));
}

function buildIgnitionSpawns(_carHalfHeight: number): CarSpawn[] {
	...
	for (let i = 0; i < 8; i++) {
		...
		spawns.push({
			slotIndex: i,
			team: null,
			displayName: ignitionFfaDisplayName(i),
...
/**
 * Kickoff Meridian — auta na powierzchni sfery blisko bieguna dolnego,
 * blue (−Z) / orange (+Z), nosem do środka (piłka).
 */
function buildMeridianSpawns(carHalfHeight: number): CarSpawn[] {
	const R = meridianSphereRadius();
	const centerY = R;
	const ride = Math.max(0.45, carHalfHeight + 0.08);
	/** ~18° od nadiru — niski punkt, swobodny wjazd na ściany. */
	const fromNadir = 0.32;
	const slots: { team: ScoringTeam; azimuth: number; role: SpawnRole }[] = [
		{ team: "blue", azimuth: Math.PI - 0.38, role: "offensive_corner" },
		{ team: "blue", azimuth: Math.PI + 0.38, role: "center_back" },
		{ team: "orange", azimuth: 0.38, role: "offensive_corner" },
		{ team: "orange", azimuth: -0.38, role: "center_back" },
	];
	...
	for (let i = 0; i < slots.length; i++) {
		const s = slots[i]!;
		const teamSlot = s.team === "blue" ? blueSlot++ : orangeSlot++;
		const dirX = Math.sin(fromNadir) * Math.sin(s.azimuth);
		const dirY = -Math.cos(fromNadir);
		const dirZ = Math.sin(fromNadir) * Math.cos(s.azimuth);
		const pos = new THREE.Vector3(
			dirX * (R - ride),
			centerY + dirY * (R - ride),
			dirZ * (R - ride),
		);
		/** Nos w stronę środka boiska (0, ·, 0) w XZ. */
		const yaw = Math.atan2(-pos.x, -pos.z);
		const isHuman = s.team === "blue" && teamSlot === 0;
		spawns.push({
			slotIndex: i,
			team: s.team,
			displayName: isHuman
