...
/** Pełny stan kinematyczny — anchor input replay. */
export function applyFullBodySnapshot(
	body: RAPIER.RigidBody,
	meshRoot: THREE.Object3D,
	snap: BodySnapshot,
): void {
	applyVisualSnapshot(body, meshRoot, snap);
	if (snap.lvx !== undefined) {
		body.setLinvel({ x: snap.lvx, y: snap.lvy!, z: snap.lvz! }, true);
		body.setAngvel({ x: snap.avx!, y: snap.avy!, z: snap.avz! }, true);
	}
}
...
export function lerpSnapshot(
	...
	if (a.lvx !== undefined && b.lvx !== undefined) {
		out.lvx = THREE.MathUtils.lerp(a.lvx, b.lvx, u);
		out.lvy = THREE.MathUtils.lerp(a.lvy!, b.lvy!, u);
		out.lvz = THREE.MathUtils.lerp(a.lvz!, b.lvz!, u);
	}
	if (a.avx !== undefined && b.avx !== undefined) {
		out.avx = THREE.MathUtils.lerp(a.avx, b.avx, u);
		out.avy = THREE.MathUtils.lerp(a.avy!, b.avy!, u);
		out.avz = THREE.MathUtils.lerp(a.avz!, b.avz!, u);
	}
	return out;
}
...
export const DEFAULT_REPLAY_PLAYBACK: Required<ReplayPlaybackOptions> = {
	goalCrossTime: 0,
	baseSpeed: 0.92,
	goalSlowMul: 0.11,
	goalSlowBefore: 1.05,
...
export class GoalReplayRecorder {
	...
	buildClip(fromT: number, toT: number, goalT?: number): ReplayFrame[] {
		const { frames } = this.buildClipRaw(fromT, toT, goalT);
		return frames;
	}

	/** Pierwsza klatka klipu — anchor dla input replay. */
	getAnchorFrame(fromT: number, toT: number): ReplayFrame | null {
		const { frames } = this.buildClipRaw(fromT, toT);
		return frames[0] ?? null;
	}

	/**
	 * Czas przekroczenia linii względem startu klipu (po normalizacji t0=0).
	 * Gdy bufor nie sięga pełnego PRE, nie zakładamy sztucznych 5.5 s.
	 */
	getClipGoalCrossTime(fromT: number, toT: number, goalT: number): number {
		const { goalCrossTime } = this.buildClipRaw(fromT, toT, goalT);
		return goalCrossTime;
	}

	private buildClipRaw(
		fromT: number,
		toT: number,
		goalT?: number,
	): { frames: ReplayFrame[]; goalCrossTime: number } {
		if (this.frames.length === 0) {
			return { frames: [], goalCrossTime: 0 };
		}
		...
		const t0 = clip[0]!.t;
		const crossAbs = goalT ?? toT - 0.01;
		const goalCrossTime = THREE.MathUtils.clamp(crossAbs - t0, 0.05, span);
		return {
			frames: normalizeClipTimes(clip),
			goalCrossTime,
		};
	}
}
...
export class GoalReplayPlayer {
	...
	getProgress(): number {
		return this.duration > 0
			? THREE.MathUtils.clamp(this.elapsed / this.duration, 0, 1)
			: 1;
	}
	...
	ballVelocity(out: THREE.Vector3): THREE.Vector3 {
		...
		const snap = this.sampleBall(this.elapsed);
		if (snap.lvx !== undefined) {
			return out.set(snap.lvx, snap.lvy!, snap.lvz!);
		}
