...
function scaleRlXZ(xUu: number, zUu: number): { x: number; z: number } {
	return {
		x: (xUu / 100) * (RL_ARENA.HALF_WIDTH / RL_HALF_WIDTH_M),
		z: (zUu / 100) * (RL_ARENA.HALF_LENGTH / RL_HALF_LENGTH_M),
	};
}
...
const MODE_LAYOUT: Record<1 | 2 | 3 | 4, { blue: string[]; orange: string[] }> =
	{
		1: { blue: ["rightCorner"], orange: ["leftCorner"] },
		2: { blue: ["rightCorner", "farBack"], orange: ["leftCorner", "farBack"] },
		3: {
...
function slotToSpawn(
	...
	return {
		...
		displayName: isHuman
			? "You"
			: `${team === "blue" ? "Blue" : "Orange"} ${teamSlot + 1}`,
