...
export function buildWorldSnapshot(
	...
	return {
		...
		playerStats: match.scoring.exportPayload().map((row) => ({
			slot: row.slotIndex,
			score: row.score,
			goals: row.goals,
			assists: row.assists,
...
export type ApplySnapshotOptions = {
	/** Sloty pomijane — client-side prediction gościa. */
	skipSlots?: number[];
};
...
export function applyWorldSnapshot(
	...
	for (const carSnap of snapshot.cars) {
		if (skip.has(carSnap.slot)) continue;
	...
	if (match && snapshot.playerStats?.length) {
		match.scoring.applyPayload(
