...
export class HitVfx {
	...
	trigger(
		point: THREE.Vector3,
		impact: number,
		opts?: { team?: "blue" | "orange"; normal?: THREE.Vector3 },
	): void {
		if (impact < 3.2) return;

		const mat = this.sparkLines.material as THREE.LineBasicMaterial;
		if (opts?.team === "blue") mat.color.setHex(0x66ddff);
		else if (opts?.team === "orange") mat.color.setHex(0xffaa55);
		else mat.color.setHex(0x00ffcc);

		const speedScale = THREE.MathUtils.clamp(impact / 20, 0.4, 2.8);
		const bias = opts?.normal;
		for (let i = 0; i < SPARK_COUNT; i++) {
			...
			if (bias && bias.lengthSq() > 1e-6) {
				const spread = 0.55 + Math.random() * 0.9;
				slot.velocity
					.copy(bias)
					.normalize()
					.multiplyScalar((10 + Math.random() * 22) * speedScale * spread);
				slot.velocity.x += (Math.random() - 0.5) * 6;
				slot.velocity.y += 2 + Math.random() * 8;
				slot.velocity.z += (Math.random() - 0.5) * 6;
			} else {
