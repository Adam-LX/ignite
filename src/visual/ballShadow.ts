...
export class BallShadow {
	...
	update(ballPos: THREE.Vector3, floorY = 0.055): void {
		...
		const contact = THREE.MathUtils.clamp(1 - height / 1.35, 0, 1);
		const scale = THREE.MathUtils.lerp(1, 0.55, t ** 0.85);
		const scaleContact = THREE.MathUtils.lerp(
			scale,
			scale * 0.68,
			contact * 0.72,
		);
		const opacity = THREE.MathUtils.lerp(0.68, 0.22, t ** 1.1);
		const opacityContact = THREE.MathUtils.lerp(opacity, 0.86, contact * 0.58);

		this.mesh.scale.set(scaleContact, scaleContact, 1);
		(this.mesh.material as THREE.MeshBasicMaterial).opacity = opacityContact;
	}
}
