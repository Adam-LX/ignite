...
export class CameraIntroTransition {
	...
	update(
		...
		/** Wall-clock — rAF w tle / headless dławi dt i intro wisiało na pozie menu. */
		const wall = (performance.now() - this.startMs) / 1000;
		this.elapsed = Math.max(this.elapsed + dt, wall);
