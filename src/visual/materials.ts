...
import { installGrassMowPattern } from "./grassMowPattern";
import {
...
void main() {
	...
	/** Fresnel — rim przy krawędzi sylwetki (czytelność na boisku). */
	float ndotv = max(0.0, dot(viewN, viewDir));
	float fresnel = pow(1.0 - ndotv, uFresnelPower);
	fresnel *= mix(0.85, 1.25, clamp(uHitFlash * 0.22, 0.0, 1.0));
	col += uFresnelColor * fresnel * uFresnelIntensity;
...
function getBallAlbedoUrl(): string {
	if (getMeshyArenaManifest()?.ballAlbedo) {
		return resolveMeshyTexture(
			"ballAlbedo",
			"/assets/textures/ball_cyber_albedo.jpg",
		);
	}
	return BALL_ALBEDO_URL;
}
...
export function pitchFloorMaterial(): THREE.MeshStandardMaterial {
	...
	const useMeshyGrass = !!manifest?.grassColor;
	const loader = new THREE.TextureLoader();
	const color = loader.load(
		resolveMeshyTexture("grassColor", "/assets/textures/grass_color.jpg"),
	);
	const normal = loader.load(
		resolveMeshyTexture("grassNormal", "/assets/textures/grass_normal.jpg"),
	);
	const detailRoughness = loader.load(
		resolveMeshyTexture(
			"grassRoughness",
			"/assets/textures/grass_roughness.jpg",
		),
	);
	...
	setupGrassMap(
		...
		useMeshyGrass ? 0.18 : 0.31,
	);
	normal.repeat.copy(color.repeat);

	setupPitchDetailMap(detailRoughness);
	if (useMeshyGrass) {
		detailRoughness.repeat.copy(color.repeat);
		setupGrassMap(detailRoughness, THREE.LinearSRGBColorSpace, 1);
	}
	...
	pitchFloorMaterialCache = new THREE.MeshStandardMaterial({
		...
		normalScale: new THREE.Vector2(
			useMeshyGrass ? 0.58 : 0.38,
			useMeshyGrass ? 0.58 : 0.38,
		),
		roughnessMap: detailNoise,
		aoMap: useMeshyGrass ? detailRoughness : detailRoughness,
		aoMapIntensity: useMeshyGrass ? 0.18 : 0.22,
		roughness: useMeshyGrass ? 0.9 : 0.93,
		metalness: 0,
		/** Jaśniejszy tint — mapa Meshy bywa ciemna w ACES. */
		color: useMeshyGrass ? 0xe8f0e4 : 0xf0f4ee,
		envMapIntensity: useMeshyGrass ? 0.12 : 0.08,
	});

	if (!useMeshyGrass) {
		installGrassMowPattern(pitchFloorMaterialCache);
	}
...
export function createBallMaterial(): THREE.ShaderMaterial {
	...
	ballMaterialCache = new THREE.ShaderMaterial({
		uniforms: {
			...
			uFresnelPower: { value: 2.85 },
			uFresnelIntensity: { value: 0.95 },
			uFresnelColor: { value: new THREE.Color(0.45, 0.92, 1.0) },
		},
...
export function enhanceBall(
	root: THREE.Object3D,
	keepMeshyGeometry = false,
): void {
	if (keepMeshyGeometry) {
		const manifest = getMeshyArenaManifest();
		const meshyAlbedo = manifest?.ballAlbedo
			? getCachedMeshyTexture("ballAlbedo")
			: null;
		...
		root.traverse((obj) => {
			...
			mat.roughness = Math.max(0.12, mat.roughness * 0.85);
			mat.envMapIntensity = 2.8;
			if (mat.emissiveIntensity < 0.35) {
				mat.emissive = new THREE.Color(0x0a1420);
				mat.emissiveIntensity = 0.35;
			}
			if (meshyAlbedo && !mat.map) {
				mat.map = meshyAlbedo;
			}
			if (mat instanceof THREE.MeshPhysicalMaterial) {
				mat.clearcoat = Math.max(mat.clearcoat, 0.72);
				mat.clearcoatRoughness = 0.06;
				mat.iridescence = Math.max(mat.iridescence, 0.35);
			}
			obj.material = mat;
		});
		root.visible = true;
...
export function ledBannerMaterial(
	...
	const manifest = getMeshyArenaManifest();
	const hasBanner = !!manifest?.bannerPanel;
	const mat = new THREE.MeshStandardMaterial({
		color: hasBanner ? 0xffffff : 0x060a12,
		emissive: new THREE.Color(emissive),
		emissiveIntensity: hasBanner ? 3.2 : 5.0,
		roughness: hasBanner ? 0.28 : 0.35,
		metalness: hasBanner ? 0.72 : 0.55,
		envMapIntensity: hasBanner ? 1.8 : 1.0,
	});
	if (hasBanner) {
		const cached = getCachedMeshyTexture("bannerPanel");
		const tex =
			cached ??
			new THREE.TextureLoader().load(resolveMeshyTexture("bannerPanel", ""));
		tex.colorSpace = THREE.SRGBColorSpace;
		tex.wrapS = THREE.RepeatWrapping;
		tex.wrapT = THREE.RepeatWrapping;
...
/** Wewnętrzny sufit areny — prawie przezroczysty, tylko subtelny marker (widać mecz pod spodem). */
...
export function arenaCeilingMaterial(): THREE.MeshStandardMaterial {
	if (!arenaCeilingMaterialCache) {
		arenaCeilingMaterialCache = new THREE.MeshStandardMaterial({
			color: 0x0a1528,
			emissive: new THREE.Color(0x1a3a58),
...
/** Cienka obwódka sufitu — czytelny „sufit jest tu”, bez zasłaniania. */
export function arenaCeilingEdgeMaterial(): THREE.LineBasicMaterial {
