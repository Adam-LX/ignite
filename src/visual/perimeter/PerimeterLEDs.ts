...
const whiteLedMat = new THREE.MeshStandardMaterial({
	color: 0xffffff,
	emissive: 0xffffff,
	/** Baza; realna wartość co klatkę w `update` (niżej = miększy bloom). */
...
/** Dolny pasek na powierzchni ćwiartki (blisko murawy). */
const BOTTOM_STRIP_T = 0.05;
/** Górny pasek — wysoko na ćwiartce, tuż przed pionem (nie półka na ścianie). */
const TOP_STRIP_T = 0.92;
...
function buildEdgeStripGeometry(
	...
	const t = edge === "bottom" ? BOTTOM_STRIP_T : TOP_STRIP_T;
	const run0 = rampCurveRun(t);
	const y0 = RAMP_BASE_Y + rampCurveHeight(t);
	const run1 = rampCurveRun(Math.min(1, t + 0.035));
	const y1 = RAMP_BASE_Y + rampCurveHeight(Math.min(1, t + 0.035));
	...
	for (let i = 0; i < n; i++) {
		...
		const a = ribbon[i];
		const b = ribbon[j];
		const na = rampSurfaceNormal(t, a.outX, a.outZ);
		const nb = rampSurfaceNormal(t, b.outX, b.outZ);
		const lift = LED_STRIP_LIFT;

		const a0x = a.x + a.outX * run0 + na.x * lift;
		const a0y = y0 + na.y * lift;
		const a0z = a.z + a.outZ * run0 + na.z * lift;
		const a1x = a.x + a.outX * run1 + na.x * lift;
		const a1y = y1 + na.y * lift;
		const a1z = a.z + a.outZ * run1 + na.z * lift;
		const b0x = b.x + b.outX * run0 + nb.x * lift;
		const b0y = y0 + nb.y * lift;
		const b0z = b.z + b.outZ * run0 + nb.z * lift;
		const b1x = b.x + b.outX * run1 + nb.x * lift;
		const b1y = y1 + nb.y * lift;
		const b1z = b.z + b.outZ * run1 + nb.z * lift;
...
export class PerimeterLEDs {
	...
	private addPointLights(stadium: THREE.Group, ribbon: RibbonVertex[]): void {
		...
		const n = ribbon.length;
		const lightY = RAMP_BASE_Y + RAMP_RUN * 0.55;
	...
	update(_timeSec: number): void {
		/** ~0.55–0.75 — pasek widoczny, bez bloom-flashbanga (bramki osobno). */
		const basePulse = 0.65 + Math.sin(Date.now() * 0.005) * 0.12;
		const surgeBoost =
			surgeStrength > 0
				? surgeStrength *
					(0.65 + 0.35 * Math.sin(Date.now() * 0.012 + surgeWave * 3.5))
				: 0;
		const pulse = basePulse + surgeBoost * 1.8;
