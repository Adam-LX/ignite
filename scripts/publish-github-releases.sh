...
collect_assets() {
	...
	if [[ ! -f "${src}" ]]; then
		export_source_tarball >/dev/null 2>&1 || true
	fi
	if [[ ! -f "${src}" ]]; then
		return 0
	fi
	...
	local src_mb
	src_mb=$(( $(stat -c%s "${src}" 2>/dev/null || stat -f%z "${src}") / 1024 / 1024 ))
	if (( src_mb > 1900 )); then
		echo "Pomijam src (${src_mb} MB > limit GitHub 2 GiB): ${src}" >&2
	else
		printf '%s\n' "${src}"
	fi
}
