...
def load_policy_into(model: PolicyMLP, path: Path) -> float:
    ...
    with torch.no_grad():
        w2 = data["w2"]
        b2 = data["b2"]
        old_hidden = len(data["b1"])
        old_outputs = len(b2)
        if old_hidden != HIDDEN or old_outputs != OUTPUT:
            new_w2: list[float] = []
            new_b2: list[float] = []
            for o in range(OUTPUT):
                for h in range(HIDDEN):
                    if o < old_outputs and h < old_hidden:
                        new_w2.append(w2[o * old_hidden + h])
                    else:
                        new_w2.append(0.0)
                new_b2.append(b2[o] if o < old_outputs else 0.0)
            new_w1: list[float] = []
            new_b1: list[float] = []
            for h in range(HIDDEN):
                for i in range(INPUT):
                    if h < old_hidden:
                        new_w1.append(data["w1"][h * INPUT + i])
                    else:
                        new_w1.append(0.0)
                new_b1.append(data["b1"][h] if h < old_hidden else 0.0)
            model.fc1.weight.copy_(torch.tensor(new_w1, dtype=torch.float32).view(HIDDEN, INPUT))
            model.fc1.bias.copy_(torch.tensor(new_b1, dtype=torch.float32))
            model.fc2.weight.copy_(torch.tensor(new_w2, dtype=torch.float32).view(OUTPUT, HIDDEN))
            model.fc2.bias.copy_(torch.tensor(new_b2, dtype=torch.float32))
        else:
            ...
            model.fc2.weight.copy_(torch.tensor(w2, dtype=torch.float32).view(OUTPUT, HIDDEN))
