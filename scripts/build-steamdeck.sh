...
patch_nixos_binary() {
	...
	local sys="$2"
	[[ -n "${sys}" ]] || return 0
	# electron-builder może nadpisać cache świeżym 7z — zawsze wymuś wrapper NixOS
	if [[ -f "${bin}" && ! -f "${bin}.bundled" ]]; then
		mv "${bin}" "${bin}.bundled"
	elif [[ ! -f "${bin}" && ! -f "${bin}.bundled" ]]; then
		return 0
	fi
	printf '#!/usr/bin/env bash\nexec %q "$@"\n' "${sys}" >"${bin}"
	chmod +x "${bin}"
...
ensure_appimage_tools_archive() {
	local cache="${HOME}/.cache/electron-builder/appimage"
	local ver="12.0.1"
	local arch7z="${cache}/appimage-${ver}.7z"
	local url="https://github.com/electron-userland/electron-builder-binaries/releases/download/appimage-${ver}/appimage-${ver}.7z"
	mkdir -p "${cache}"
	if [[ ! -f "${arch7z}" ]]; then
		echo "Pobieram appimage tools ${ver}…"
		curl -fsSL -o "${arch7z}.partial" "${url}"
		mv "${arch7z}.partial" "${arch7z}"
	fi
}
...
preextract_appimage_tools() {
	...
	for arch7z in "${cache}"/appimage-*.7z; do
		dir="${arch7z%.7z}"
		if [[ ! -f "${dir}/linux-x64/mksquashfs" && ! -f "${dir}/linux-x64/mksquashfs.bundled" ]]; then
			mkdir -p "${dir}"
	...
	if ! command -v mksquashfs >/dev/null 2>&1; then
		echo "Brak mksquashfs w PATH — dodaj squashfsTools do flake (nix develop)." >&2
		exit 1
	fi
}
...
run_build() {
	...
	if ! npx electron-builder --linux AppImage --x64 --publish never; then
		echo "AppImage nieudany — ponawiam patch NixOS (świeży download tools?)…"
		preextract_appimage_tools
