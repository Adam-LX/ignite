...
| **Źródła** | [Ignite-${version}-src.tar.zst](${base}/Ignite-${version}-src.tar.zst) | ~133 MB |
...
### Ze źródeł
\`\`\`bash
tar -xf Ignite-${version}-src.tar.zst
cd ignite-${version}-src
...
Binaria desktopowe i archiwum źródeł. Kod gry: [README.md](README.md).
...
| **Źródła** | [Ignite-${version}-src.tar.zst](https://codeberg.org/${CODEBERG_USER}/${CODEBERG_REPO}/media/tag/v${version}-src/releases/Ignite-${version}-src.tar.zst) | kod gry (bez node_modules) |
