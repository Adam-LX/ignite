...
function simulateCarBallHit(
	...
	for (let i = 0; i < frames; i++) {
		beforeStep?.();
		snapshotBallKinematics(ball);
		scene.advancePhysics(FRAME_DT, (fixedDt, substep, substepCount) => {
...
describe("Balans piłki", () => {
	...
	it("delikatne szturchnięcie (~3 m/s) porusza piłkę", () => {
		...
		for (let i = 0; i < 35; i++) {
			...
			const v = ball.rapierRigidBody.linvel();
			const horiz = Math.hypot(v.x, v.z);
			if (horiz > 0.4) sawHit = true;
			if (sawHit) {
				peakHoriz = Math.max(peakHoriz, horiz);
				peakVy = Math.max(peakVy, Math.abs(v.y));
			}
			if (sawHit && i > 22) break;
		}

		expect(sawHit).toBe(true);
		expect(peakHoriz).toBeGreaterThan(0.8);
		expect(peakHoriz).toBeLessThan(12);
		expect(peakVy).toBeLessThan(5);
	});

	it("soft tap (~2.2 m/s) — niski Δv, bez wystrzału w górę", () => {
		const scene = new Scene();
		buildArenaPhysics(scene.rapierWorld);
		const ball = createBall(scene);
		ball.rapierRigidBody.setTranslation(
			{ x: 0, y: RL_BALL.radius + 0.08, z: -3.5 },
			true,
		);

		const carMesh = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.7, 3.6));
		const car = new RocketCar(scene, carMesh);
		car.rapierRigidBody.setTranslation({ x: 0, y: 1.4, z: -5.8 }, true);
		car.rapierRigidBody.setLinvel({ x: 0, y: 0, z: 2.2 }, true);
		...
		for (let i = 0; i < 30; i++) {
			...
			ball.syncWithRigidBody();
			const v = ball.rapierRigidBody.linvel();
			peakHoriz = Math.max(peakHoriz, Math.hypot(v.x, v.z));
			peakVy = Math.max(peakVy, Math.abs(v.y));
		}

		expect(peakHoriz).toBeGreaterThan(0.45);
		expect(peakHoriz).toBeLessThan(10);
		expect(peakVy).toBeLessThan(4);
	});
	...
	it("odbija się od ściany bocznej", () => {
		...
		for (let i = 0; i < Math.round(1.5 / FRAME_DT); i++) {
			...
			if (vx < 0) maxReboundSpeed = Math.max(maxReboundSpeed, -vx);
		}

		const v = ball.rapierRigidBody.linvel();
		expect(minX).toBeLessThan(RL_ARENA.HALF_WIDTH - margin);
		expect(v.x).toBeLessThan(-2);
		expect(maxReboundSpeed).toBeGreaterThan(4);
	});

	it("bounce z murawy — hang w oknie CR≈0.6", () => {
		const scene = new Scene();
		buildArenaPhysics(scene.rapierWorld);
		const ball = createBall(scene);
		ball.rapierRigidBody.setTranslation({ x: 0, y: 4, z: 0 }, true);
		ball.rapierRigidBody.setLinvel({ x: 0, y: 0, z: 0 }, true);
		...
		for (let i = 0; i < Math.round(4 / FRAME_DT); i++) {
			...
			if (
				touchedGround &&
				vy < -1 &&
				firstBouncePeak > RL_BALL.radius + 0.3
			) {
				break;
			}
		}

		expect(touchedGround).toBe(true);
		expect(firstBouncePeak).toBeGreaterThan(1.7);
		expect(firstBouncePeak).toBeLessThan(2.85);
	});
	...
	it("updateBallPhysics nie przekracza RL_BALL.maxSpeed", () => {
		const scene = new Scene();
		buildArenaPhysics(scene.rapierWorld);
		const ball = createBall(scene);
		ball.rapierRigidBody.setLinvel({ x: 40, y: 12, z: 35 }, true);
		...
		expect(speed).toBeLessThanOrEqual(RL_BALL.maxSpeed + 0.05);
	});
});
