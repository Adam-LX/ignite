...
describe("room server online flow", () => {
	...
	it("GET /rooms zwraca otwarte pokoje po createRoom", async () => {
		...
		expect(body.rooms.some((r) => r.code === welcome.roomCode)).toBe(true);
	...
	it("forfeit — gracz poddaje mecz, obaj dostają matchForfeit", async () => {
		const host = await connectWs();
		const guest = await connectWs();
		const hostIn = attachCollector(host);
		const guestIn = attachCollector(guest);
		...
		send(guest, {
			type: "joinRoom",
			roomCode: welcome.roomCode,
			clientId: `guest_fx_${randomUUID()}`,
		});
		await hostIn.waitFor((m) => m.type === "startMatch");
		await guestIn.waitFor((m) => m.type === "startMatch");

		send(guest, { type: "forfeit" });

		const hostForfeit = (await hostIn.waitFor(
			(m) => m.type === "matchForfeit",
		)) as Extract<ServerMessage, { type: "matchForfeit" }>;
		const guestForfeit = (await guestIn.waitFor(
			(m) => m.type === "matchForfeit",
		)) as Extract<ServerMessage, { type: "matchForfeit" }>;

		expect(hostForfeit.loserSlot).toBe(1);
		expect(hostForfeit.blueScore).toBe(3);
		expect(hostForfeit.orangeScore).toBe(0);
		expect(guestForfeit).toEqual(hostForfeit);

		const hostRanked = (await hostIn.waitFor(
			(m) => m.type === "rankedResult",
		)) as Extract<ServerMessage, { type: "rankedResult" }>;
		const guestRanked = (await guestIn.waitFor(
			(m) => m.type === "rankedResult",
		)) as Extract<ServerMessage, { type: "rankedResult" }>;
		expect(hostRanked.delta).toBeGreaterThan(0);
		expect(guestRanked.delta).toBeLessThan(0);
	...
	it("createRoom 2v2 — startMatch po 4 graczach", async () => {
		...
		const collectors = sockets.map((ws) => attachCollector(ws));

		send(sockets[0]!, {
			type: "createRoom",
			mode: "2v2",
			ranked: false,
			clientId: `host_2v2_${randomUUID()}`,
		});
		const welcome = (await collectors[0]!.waitFor(
			(m) => m.type === "welcome",
		...
		for (let i = 1; i < 4; i++) {
			send(sockets[i]!, {
				type: "joinRoom",
				roomCode: welcome.roomCode,
				clientId: `guest_2v2_${i}_${randomUUID()}`,
			});
			await collectors[0]!.waitFor((m) => m.type === "peerJoined");
		}

		const starts = await Promise.all(
			collectors.map((c) =>
				c.waitFor((m) => m.type === "startMatch") as Promise<
					Extract<ServerMessage, { type: "startMatch" }>
				>,
			),
		);
		for (const start of starts) {
			expect(start.mode).toBe("2v2");
		}

		const roomsRes = await fetch(`http://127.0.0.1:${PORT}/rooms`);
		const roomsBody = (await roomsRes.json()) as {
			rooms: { code: string; mode?: string }[];
		};
		expect(
			roomsBody.rooms.some((r) => r.code === welcome.roomCode),
		).toBe(false);
