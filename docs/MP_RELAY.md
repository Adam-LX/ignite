...
Menu → **Online** (wybrany tryb z karuzeli) → party lobby (Ready / Start) → pre-match pads → mecz.
Wszystkie tryby menu; puste sloty = boty u hosta. Ranked tylko 1v1/2v2. W LAN gość wpisuje `192.168.x.x:8765`.

### Lobby (2 etapy)

1. **Party** — `MultiplayerLobby`: tryb, lista Blue/Orange, Ready, host Start (`requestStart`).
2. **Pre-match** — `PreMatchLobbyScene`: pady 3D + countdown (~4 s) → `GameSession.createOnline`.

Protokół: `lobbyState`, `setReady`, `setLoadout`, `requestStart`; `MP_MAX_SLOTS = 8`.
